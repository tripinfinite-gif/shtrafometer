...
- ✅ Автофиксы: SSH/FTP, 8 генераторов HTML/JS
...
- ✅ `/public/og-image.png` создан (1200×630 px, тёмный фон, фиолетовый #6C5CE7, статы 35+/8 законов/18 млн ₽)
- ❗ SMS.ru API key (`SMS_RU_API_KEY`) не настроен — OTP-коды видны только в docker logs
- ❗ YooKassa аккаунт ожидает активации

### Что делаем дальше (v2)
Phase A: Freemium-гейтинг (blur на 4+ нарушении, Compliance Score)
Phase B: Автодетекция CMS + расчёт цены исправления
Phase C: Кабинет v2 (site-centric архитектура как у Cloudflare)
Phase D: «Исправим за вас» с авто-ценой
Phase E: Мониторинг + email-отчёты (подписка 490/мес)
Подробнее: `docs/MASTER-PLAN.md` → раздел «Этапы реализации v2»
