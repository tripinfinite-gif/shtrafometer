...
export default function RegisterPage() {
  ...
  async function handleSendCode() {
    ...
    try {
      ...
      const body = isEmail ? { email, name: name.trim() } : { phone, name: name.trim() };
  ...
  async function handleVerifyCode(codeStr?: string) {
    ...
    try {
      ...
      const body = isEmail
        ? { email, code: codeValue, name: name.trim() }
        : { phone, code: codeValue, name: name.trim() };
  ...
  return (
    <div>
      ...
      {step === 'info' ? (
        <>
          <p className="text-gray-500 mb-6">Создайте аккаунт за 15 секунд</p>
          ...
          {(YANDEX_ENABLED || VK_ENABLED) && (
            <div className="space-y-3 mb-6">
              {YANDEX_ENABLED && (
                <a
                  href={`/api/auth/yandex?returnUrl=${oauthReturnUrl}`}
              ...
              {VK_ENABLED && (
                <a
                  href={`/api/auth/vk?returnUrl=${oauthReturnUrl}`}
                  className="flex items-center justify-center gap-3 w-full h-12 rounded-xl border border-gray-200 bg-white
                             text-gray-800 font-medium text-sm hover:bg-gray-50 active:bg-gray-100 transition-colors"
                >
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-gray-400">или с кодом</span>
            <div className="flex-1 h-px bg-gray-200" />
          </div>
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              disabled={loading || !name.trim() || !inputValid}
